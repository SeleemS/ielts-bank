...
describe('average user band display', () => {
  ...
  it('never calls a fallback value a submitted average when the stored aggregate is missing', () => {
    for (const averageUserBand of [null, undefined, 'invalid', -1, 10]) {
      const result = resolveAverageUserBand({ id: 'missing', difficulty: 'medium', averageUserBand, submissionCount: 4 });

...
// ---------------------------------------------------------------------------
function mockAdmin({ userRow = { id: 'user-1' } } = {}) {
...
describe('handleStripeEvent', () => {
  ...
  it('charge.refunded and disputes revoke premium immediately', async () => {
    for (const type of ['charge.refunded', 'charge.dispute.created']) {
      const { admin, calls } = mockAdmin();
      const out = await handleStripeEvent(event(type, { customer: 'cus_123' }), {
        admin,
        stripe: {},
      });
      expect(out).toContain('refunded');
      const userUpdate = calls.updates.find((u) => u.table === 'users');
      expect(userUpdate.fields).toMatchObject({ plan: 'free', plan_status: 'refunded' });
    }
  });

...
describe('resolveLookupKey', () => {
  ...
  it('leads with the pass in India, China, Hong Kong and every PPP country only', () => {
    for (const country of ['IN', 'in', 'CN', 'HK', 'PK', 'BD', 'VN', 'EG', 'NG', 'PH']) {
      expect(isPassFirstMarket(country), country).toBe(true);
    }
    for (const country of ['US', 'GB', 'SG', 'DE', 'AE', 'SA', 'TW', '', undefined]) {
      expect(isPassFirstMarket(country), String(country)).toBe(false);
    }
    // CN/HK get the pass-first layout but keep global prices.
    expect(resolveLookupKey('exam_pass', 'CN')).toBe('premium_exam_pass');
    expect(resolveLookupKey('exam_pass', 'HK')).toBe('premium_exam_pass');
  });
    ...
    async rpc(name, args) {
      ...
      if (args.p_fields.plan_sku === 'exam_pass') {
        args.p_fields = { ...args.p_fields,
          ...
          // Mirrors billing_private.fulfill_checkout: the caller-selected
          // allowlisted length, 30 days when absent.
          plan_expires_at: new Date(Date.now()
            + (Number(args.p_fields._exam_pass_days) || 30) * 86400000).toISOString(),
        };
      }
...
describe('handleStripeEvent', () => {
  ...
  it.each([
    [{ sku: 'exam_pass', pass_days: '45' }, 45],
    [{ sku: 'exam_pass', pass_days: '30' }, 30],
    // Sessions created before checkout stamped a length get today's length.
    [{ sku: 'exam_pass' }, 30],
    // A tampered/unknown stamp is never forwarded to the database.
    [{ sku: 'exam_pass', pass_days: '365' }, 30],
  ])('grants the pass length the checkout advertised (%o)', async (metadata, days) => {
    const { admin, calls } = mockAdmin();
    await handleStripeEvent(event('checkout.session.completed', {
      id: 'cs_len', mode: 'payment', payment_status: 'paid', amount_total: 1499,
      client_reference_id: 'user-1', metadata,
    }), { admin, stripe: currentStripe() });
    expect(calls.rpcs[0].args.p_fields._exam_pass_days).toBe(days);
    const granted = (new Date(calls.updates[0].fields.plan_expires_at).getTime() - Date.now()) / 86400000;
    expect(Math.round(granted)).toBe(days);
  });

  it('logs loudly if the database grants a shorter pass than advertised', async () => {
    const errors = vi.spyOn(console, 'error').mockImplementation(() => {});
    const { admin } = mockAdmin({ rpcResult: {
      status: 'applied', access_expires_at: new Date(Date.now() + 30 * 86400000).toISOString(),
    } });
    await handleStripeEvent(event('checkout.session.completed', {
      id: 'cs_short', mode: 'payment', payment_status: 'paid', amount_total: 1499,
      client_reference_id: 'user-1', metadata: { sku: 'exam_pass', pass_days: '45' },
    }), { admin, stripe: currentStripe() });
    expect(errors).toHaveBeenCalledWith('EXAM PASS LENGTH MISMATCH:', expect.objectContaining({
      sessionId: 'cs_short', promisedDays: 45,
    }));
    errors.mockRestore();
  });

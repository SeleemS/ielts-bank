...
describe('pricing SEO', () => {
  it('publishes a complete canonical social-card contract', () => {
    expect(PRICING_SEO.title).toContain('IELTS Bank Premium');
    ...
    expect(PRICING_SEO.ogImage).toBe(
      'https://www.ielts-bank.com/api/og?title=Know%20exactly%20what%20is%20holding%20your%20IELTS%20band%20back&type=pricing&subtitle=Premium'
    );
    expect(PRICING_SEO.imageAlt).toContain('IELTS Bank Premium');
  });
  ...
  it('keeps every dynamic image parameter URL-safe', () => {
    ...
    expect(url.searchParams.get('subtitle')).toBe('Premium');
  });
});

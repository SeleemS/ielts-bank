...
describe('sendGa4Purchase', () => {
  ...
  it('includes a recent GA session and omits stale or application session IDs', async () => {
    process.env.GA4_MP_API_SECRET = 'secret';
    const fetchMock = vi.fn().mockResolvedValue({ status: 204 });
    vi.stubGlobal('fetch', fetchMock);
    const recent = String(Math.floor(Date.now() / 1000) - 60);
    for (const sessionId of [recent, String(Number(recent) - 86400), 'application-uuid', null]) {
      await sendGa4Purchase({ clientId: '1.2', sessionId, transactionId: 'cs_same', amountMinor: 899, currency: 'usd', sku: 'monthly' });
    }
    const events = fetchMock.mock.calls.map(([, init]) => JSON.parse(init.body).events[0].params);
    expect(events[0].session_id).toBe(recent);
    events.slice(1).forEach((params) => expect(params).not.toHaveProperty('session_id'));
    events.forEach((params) => expect(params.transaction_id).toBe('cs_same'));
  });

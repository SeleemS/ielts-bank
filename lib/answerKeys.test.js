...
describe('practicePageTitle', () => {
  it('puts the passage title first and mentions answers only when a key page exists', () => {
    expect(practicePageTitle('Why the Body Needs Vitamins', 'reading', true)).toBe(
      'Why the Body Needs Vitamins | IELTS Reading Practice & Answers'
    );
    expect(practicePageTitle('Booking an Airport Taxi', 'listening', true)).toBe(
      'Booking an Airport Taxi | IELTS Listening Practice & Answers'
    );
    expect(practicePageTitle('Endless Harvest', 'reading', false)).toBe(
      'Endless Harvest | IELTS Reading Practice'
    );
    expect(practicePageTitle('', 'reading', true)).toBe('IELTS Reading Practice | IELTS-Bank');
  });

  it('preserves the full topic when an answers suffix would make the title too long', () => {
    const topic = 'A History of the World in Board Games';
    const result = practicePageTitle(topic, 'reading', true);
    expect(result).toBe(`${topic} | IELTS Reading Practice`);
    expect(result.length).toBeLessThanOrEqual(70);
    expect(practicePageTitle('Short passage', 'reading', false)).not.toContain('Answers');
    expect(practicePageTitle('  Aphantasia: A life without mental images    ', 'reading', false)).toBe(
      'Aphantasia: A life without mental images | IELTS Reading Practice'
    );
    expect(practicePageTitle('   ', 'reading', false)).toBe('IELTS Reading Practice | IELTS-Bank');
  });
});

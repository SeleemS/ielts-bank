...
// ---------------------------------------------------------------------------
// Writing: a short MEASURED sample — the default, replacing self-assessment.
//
// ~100 words keeps the estimator a quick diagnostic while still giving enough
// language to judge Grammatical Range and Lexical Resource reliably (Task
// Response and Coherence are necessarily limited by length, which is why the
// result is presented as an INDICATIVE band). The band is scored server-side and
// withheld from the anonymous client until sign-up — see
// /api/estimator/score-writing — which makes it the tool's main conversion
// lever. WRITING_SELF_ASSESSMENT below remains the fallback for visitors who
// choose to skip the sample.
// ---------------------------------------------------------------------------
export const WRITING_SAMPLE_TASK = {
  prompt:
    'Some people think students should be allowed to use mobile phones in school. Do you agree or disagree?',
  guidance: 'Write about 100 words — roughly 5 minutes.',
  targetWords: 100,
  minWords: 40,
  maxWords: 180,
};

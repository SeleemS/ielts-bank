...
function passageListSelect(includeBandStats, includeQuestionGroups) {
  return [
    'slug',
    'legacy_firestore_id',
    'title',
    ...
    ...(includeQuestionGroups ? ['question_groups!inner(question_type)'] : []),
  ].join(', ');
}
...
async function fetchPassageListRows(skill, questionType = null) {
  const run = (includeBandStats) => {
    let query = getSupabase()
      .from('passages')
      .select(passageListSelect(includeBandStats, Boolean(questionType)))
      .eq('skill', skill)
      .eq('status', 'published');
...
export async function listPassages(skill) {
  const { data, error } = await fetchPassageListRows(skill);
  if (error) throw error;

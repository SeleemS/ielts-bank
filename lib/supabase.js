...
import { answerPageEligible } from './answerKeys';
import { withoutRetiredDuplicates } from './questionUrls';
import { pickRelatedPractice } from './relatedPractice';
...
export function toStructuredPassageShape(row) {
  ...
  const droppedQuestionCount =
    groups.reduce((n, g) => n + g.questions.length, 0) -
    gradeableGroups.reduce((n, g) => n + g.questions.length, 0);
  ...
  const shaped = {
    ...
    // How many questions the integrity guard removed (0 for a complete key).
    // Answer-key pages are only published for complete passages.
...
export async function listPassagesByQuestionType(skill, questionType) {
  ...
  return withoutRetiredDuplicates(skill, out);
}
...
export async function listPassages(skill, { withQuestionTypes = false } = {}) {
  ...
  if (error) throw error;
  // Retired exact-duplicate prompts redirect to their twin, so no list links them.
  return withoutRetiredDuplicates(skill, (data || []).map(toPassageListItem));
}

// Sitemap enumeration: every published passage of a skill with the timestamps
// the sitemap uses for <lastmod>. Kept separate from listPassages so the hub
// lists do not ship timestamps they never render.
//   [{ slug, createdAt, updatedAt }]
export async function listSitemapPassages(skill) {
  const { data, error } = await getSupabase()
    ...
    .order('title', { ascending: true });
  if (error) throw error;
  return withoutRetiredDuplicates(
    skill,
    (data || []).map((row) => ({
      slug: row.slug,
...
// Every published writing question with the fields the IELTS Essay Bank hub
// (/ielts-essay-bank) classifies and previews: task, module, tags, the prompt
// (for the Task 2 frame / Task 1 visual type) and the model answer (so pages
// without one are left out). Raw rows; lib/essayBankCatalogue.js shapes them.
export async function listWritingModelAnswers() {
  const { data, error } = await getSupabase()
    .from('passages')
    .select(
      'slug, legacy_firestore_id, module, title, topic_tags, writing_details ( task, prompt_html, model_answer_html )'
    )
    .eq('skill', SKILLS.writing)
    .eq('status', 'published')
    .order('title', { ascending: true });
  if (error) throw error;
  return data || [];
...
export async function getRelatedPractice(skill, currentSlug, preferred = null, limit = 6) {
  let preferredItems = [];
  ...
  // Ring selection (lib/relatedPractice.js): the items that follow this one,
  // so internal links spread across the whole bank instead of piling onto the
  // alphabetically-first three passages.
  return pickRelatedPractice({
    preferred: preferredItems,
    all: allItems,
...
// pages' getStaticPaths, the sitemap and cross-links all agree. Memoised like
// the related-practice lists because every answers page asks for it.
...
export async function listAnswerKeySlugs(skill) {
  return cachedList(`answer-keys:${skill}`, async () => {
    const { data, error } = await getSupabase()
      .from('passages')

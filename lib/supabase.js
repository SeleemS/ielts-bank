...
export async function listWritingModelAnswers() {
  const { data, error } = await getSupabase()
    ...
    .order('title', { ascending: true });
  if (error) throw error;
  // Retired duplicate prompts 308 to their kept twin; the essay bank must
  // neither link nor count them (lib/questionUrls.js).
  return withoutRetiredDuplicates(SKILLS.writing, data || []);
}

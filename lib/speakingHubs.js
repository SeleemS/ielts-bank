...
const HUB_SELECT = `
  slug, legacy_firestore_id, title, difficulty, topic_tags, created_at, updated_at,
...
export async function listSpeakingHubItems() {
  ...
  return (data || [])
    .map((row) => {
      ...
      const routeId = row.slug;
      return {
        slug: row.slug,
        routeId,
        href: questionPath('speaking', { slug: row.slug }),
...
// (Month grouping; /ielts-speaking-cue-cards shows the newest month.)

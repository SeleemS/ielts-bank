...
import { queueCheckoutRecoveryEmail, safeRecoveryUrl, upgradeFromSession } from './checkoutRecovery';
...
async function recordRecoveredCheckout(admin, session, userId, { sku, ppp }) {
  if (!session.recovered_from) return;
  ...
  await sendGa4Event({
    ...
    params: { transaction_id: session.id, recovered_from: session.recovered_from, sku: sku || '' },
  });
}
...
export async function handleStripeEvent(event, { admin, stripe }) {
  ...
  switch (event.type) {
    ...
    case 'checkout.session.expired': {
      ...
      // checkout_abandoned email carrying Stripe's recovery link.
      const userId = await findUserId(admin, {
        userId: obj.client_reference_id || obj.metadata?.user_id,
      ...
      const emailOutcome = await queueCheckoutRecoveryEmail(admin, userId, {
        ...
        recoveryExpiresAt,
        sku,
        upgrade: upgradeFromSession(obj),
      });

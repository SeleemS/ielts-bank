 ...
 * identify the sample type without assigning every answer an unverified band; the
...
export function writingPageTitle({ title, task, module, hasModelAnswer }) {
  ...
  if (bucket === 'task1-general') {
    return `${topic} | IELTS Letter Sample`;
  }
  if (bucket === 'task1-academic') {
    return `${topic} | IELTS Task 1 Sample`;
  }
  return `${topic} | IELTS Task 2 Sample Essay`;
}

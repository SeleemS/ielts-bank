...
// Live sessions are billed per SECOND at a flat session rate — no token math,
// and silence costs the same as speech. WebRTC create bills 15 s up front
// (credited against the running session), so an abandoned session still costs
// those 15 s: every Live row floors its seconds at initSeconds.
export const LIVE_PRICES = { sessionPerMinute: 0.05, initSeconds: 15 };

function liveSessionCost(seconds) {
  return (Math.max(nonNegativeNumber(seconds), LIVE_PRICES.initSeconds) / 60) *
    LIVE_PRICES.sessionPerMinute;
}
...
export function liveReservationRow({
  ...
  const seconds = nonNegativeNumber(durationSeconds);
  // Planning allowance for the delegated Responses model, which is billed at
  // its own text rates and is not visible to us per session.
  const backendAllowanceUsd = 0.1;
  return {
    user_id: userId,
    skill: 'speaking',
    ...
    output_tokens: 0,
    audio_seconds: seconds,
    cost_usd: liveSessionCost(seconds) + backendAllowanceUsd,
    pricing_known: true,
    estimated: true,
    succeeded: true,
    input_rate_per_million: null,
    cached_input_rate_per_million: null,
    output_rate_per_million: null,
    audio_rate_per_minute: LIVE_PRICES.sessionPerMinute,
    metadata: {
      mode,
      methodology: 'reserved_duration_estimate',
      // Keep in step with LIVE_BACKEND_MODEL in lib/liveExaminer.js.
      backend_model: process.env.OPENAI_LIVE_BACKEND_MODEL || 'gpt-5.1',
      backend_allowance_usd: backendAllowanceUsd,
    },
  };
}

// Written when the session ends. The seconds are client-reported (the only
// figure we have without a usage API), so the row stays flagged estimated and
// the caller caps it against the reserved duration.
export function liveUsageRow({ userId, sessionId, usageSeconds, mode, reason = null }) {
  const seconds = nonNegativeNumber(usageSeconds);
  return {
    user_id: userId,
    skill: 'speaking',
    feature: 'speaking_live',
    ...
    provider: 'openai',
    model: 'gpt-live-1',
    provider_request_id: sessionId || null,
    input_tokens: 0,
    cached_input_tokens: 0,
    output_tokens: 0,
    audio_seconds: seconds,
    cost_usd: liveSessionCost(seconds),
    pricing_known: true,
    estimated: true,
    succeeded: true,
    input_rate_per_million: null,

...
async function speakingEntries() {
  ...
  const entries = [
    ...
    { path: '/speaking/new-cue-cards', lastmod: newestCreatedAt(cueCards) },
    ...items.map((item) => ({
...
const SECTION_LOADERS = {
  ...
  reading: async () => ({ entries: await passageEntries(SKILLS.reading, '/readingquestion'), exclude: [] }),
  writing: async (now) => ({
    entries: [...(await passageEntries(SKILLS.writing, '/writingquestion')), ...task2MonthEntries(now)],
    exclude: [],
  }),
  listening: async () => ({ entries: await passageEntries(SKILLS.listening, '/listeningquestion'), exclude: [] }),

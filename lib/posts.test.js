...
import allLegacyPosts from '../tests/fixtures/legacy-posts.json';
import { BLOG_MERGES } from './blogMerges';

// Posts deliberately merged into a hub (lib/blogMerges.js) are the one
// sanctioned way for a legacy slug to disappear: each must 308 to its hub.
const legacyPosts = allLegacyPosts.filter((post) => !BLOG_MERGES[post.slug]);
...
// Legacy posts DELIBERATELY retired by merging them into a hub page. Each one
// must carry a permanent redirect in next.config.js to where its content went,
// so removing a post here without a redirect still fails the suite.
const RETIRED = {
  'ielts-essay-bank-guide': '/ielts-essay-bank',
  'ielts-writing-bank-tips': '/ielts-essay-bank',
};
const liveLegacyPosts = legacyPosts.filter((post) => !RETIRED[post.slug]);
// Reviewed post-migration editorial updates retain the original metadata;
// the frozen migration fixture remains unchanged for historical evidence.
const EDITORIALLY_REVISED = new Set(['best-free-ielts-practice-resources']);
const REVISED_TITLES = {
  'ielts-reading-skimming-and-scanning': 'IELTS Reading: Skimming and Scanning',
  'ielts-vocabulary-health-fitness': 'IELTS Health and Fitness Vocabulary: Collocations',
  'ielts-writing-task-2-conclusion-paragraph': 'IELTS Writing Task 2: How to Write a Conclusion',
};
...
describe('blog post loader', () => {
  it('still loads every post that existed before the migration', () => {
    ...
    expect(posts.length).toBeGreaterThanOrEqual(liveLegacyPosts.length);
    const slugs = new Set(postSlugs);
    liveLegacyPosts.forEach(({ slug }) => expect(slugs.has(slug), `${slug} disappeared`).toBe(true));
    Object.keys(BLOG_MERGES).forEach((slug) =>
      expect(slugs.has(slug), `${slug} is merged into a hub but still published`).toBe(false)
    );
  ...
  it('301-redirects every retired legacy post to the page that absorbed it', async () => {
    const { default: nextConfig } = await import('../next.config.js');
    const redirects = await nextConfig.redirects();
    Object.entries(RETIRED).forEach(([slug, destination]) => {
      expect(postSlugs, `${slug} is retired but still published`).not.toContain(slug);
      expect(redirects, `${slug} has no redirect`).toContainEqual(
        expect.objectContaining({ source: `/blog/${slug}`, destination, permanent: true })
      );
    });
  });

  it.each(liveLegacyPosts.map((post) => post.slug))(
    'preserves %s with only documented editorial revisions',
    (slug) => {
      const current = withoutAddedFields(getPostBySlug(slug));
      const legacy = legacyBySlug.get(slug);
      if (EDITORIALLY_REVISED.has(slug)) {
        const { content: currentContent, ...currentMetadata } = current;
        const { content: legacyContent, ...legacyMetadata } = legacy;
        expect(currentMetadata).toEqual(legacyMetadata);
        expect(currentContent).toContain('/downloads/ielts-writing-revision-worksheet.pdf');
      } else if (REVISED_TITLES[slug]) {
        expect(current).toEqual({ ...legacy, title: REVISED_TITLES[slug] });
      } else {

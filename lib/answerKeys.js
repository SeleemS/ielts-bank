...
// Keep the complete passage name and practice context. Mention answers only
// when a published key backs the claim and the title has room for the suffix.
export function practicePageTitle(title, skill, hasAnswers) {
  const skillName = skill === 'listening' ? 'Listening' : 'Reading';
  const topic = String(title || '').replace(/\s+/g, ' ').trim();
  if (!topic) return `IELTS ${skillName} Practice | IELTS-Bank`;
  const base = `${topic} | IELTS ${skillName} Practice`;
  const withAnswers = `${base} & Answers`;
  return hasAnswers && withAnswers.length <= 70 ? withAnswers : base;
}

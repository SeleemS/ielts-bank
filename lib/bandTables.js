...
// Apply the official IELTS rounding rule to an already-computed mean of skill
// bands (mean of any number of skills). Extracted so callers that average a
// variable number of skills — e.g. the Band Estimator, which may skip a
// section — reuse the exact same rule instead of copying it. Works in
// hundredths to stay epsilon-safe on the .25 / .75 boundaries.
export function roundBandMean(mean) {
...
export function overallBand(bands) {
  ...
  return roundBandMean(valid.reduce((sum, b) => sum + b, 0) / 4);
}

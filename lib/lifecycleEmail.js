...
import { liveResumeUrl } from './checkoutRecovery';
  ...
  switch (row.email_type) {
    case 'welcome_signup':
      return {
        ...
        html: shell({
          ...
          intro: freeScoreCopy().welcomeEmailIntro,
          body:
    ...
    case 'checkout_abandoned': {
      const upgrade = ['writing', 'speaking', 'mock'].includes(payload.upgrade)
        ? payload.upgrade
        : '';
      // Our own resume link (queued by the checkout.session.expired webhook):
      // /billing/resume re-checks the account and opens a fresh checkout for
      // the same plan. NEVER link Stripe's recovery URL (payload.recovery_url)
      // — it skips every checkout guard. Rows from the cron fallback, and
      // lapsed links, point at /pricing instead.
      const recoveryUrl = liveResumeUrl(payload);
      const recoveryUntil = recoveryUrl && Date.parse(payload.resume_expires_at || '')
        ? new Date(payload.resume_expires_at).toLocaleDateString('en-US', {
          timeZone: 'UTC', month: 'long', day: 'numeric',
        })
        : '';
      const recoveryNote = recoveryUrl
        ? `<p>The button below takes you straight back to checkout for the same plan${
          payload.sku === 'exam_pass' ? ' — one payment, no auto-renew' : ''
        }. Sign in with the account you used and you’re one step from paying.${
          recoveryUntil ? ` It works until ${escapeHtml(recoveryUntil)}.` : ''
        }</p>`
        : '';
      return {
        subject: recoveryUrl
          ? 'Your IELTS Bank checkout is saved — finish in one tap'
          : 'Your Pro checkout is still open (no charge was made)',
        html: shell({
          ...
          body:
            `${recoveryNote}<p>Every plan — including the one-time Exam Pass — comes with a <strong>14-day money-back guarantee</strong>: if Pro doesn’t help, ask within 14 days and we refund it — no forms, no questions. Prices are already set for your region.</p>`,
          ctaLabel: recoveryUrl ? 'Finish my checkout' : 'Pick up where I left off',
          ctaHref: recoveryUrl || `${SITE_URL}/pricing${upgrade ? `?upgrade=${upgrade}` : ''}`,
          footer: consentFooter(
            row,

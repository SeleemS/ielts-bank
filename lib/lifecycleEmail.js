...
import { liveRecoveryUrl } from './checkoutRecovery';
  ...
  switch (row.email_type) {
    ...
    case 'checkout_abandoned': {
      const upgrade = ['writing', 'speaking', 'mock'].includes(payload.upgrade)
        ? payload.upgrade
        : '';
      // Stripe's recovery link (queued by the checkout.session.expired
      // webhook) reopens the exact checkout: same plan, same price. Rows from
      // the cron fallback, and expired links, point at /pricing instead.
      const recoveryUrl = liveRecoveryUrl(payload);
      const recoveryUntil = recoveryUrl && Date.parse(payload.recovery_expires_at || '')
        ? new Date(payload.recovery_expires_at).toLocaleDateString('en-US', {
          timeZone: 'UTC', month: 'long', day: 'numeric',
        })
        : '';
      const recoveryNote = recoveryUrl
        ? `<p>The button below reopens your checkout exactly as you left it — same plan, same price${
          payload.sku === 'exam_pass' ? ', one payment with no auto-renew' : ''
        }.${recoveryUntil ? ` It works until ${escapeHtml(recoveryUntil)}.` : ''}</p>`
        : '';
      return {
        subject: recoveryUrl

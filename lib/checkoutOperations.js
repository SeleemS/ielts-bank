import { checkoutAttribution } from './monetizationExperiment';
...
export async function recordCheckoutOperation(admin, { userId, sku, requestId, sessionId, stage, created = false, attribution }) {
  ...
  try {
    const { error } = await admin.from('activity_events').insert({
      ...
      props: { source: 'billing_checkout', operational: true, sku, stage, ...checkoutAttribution(attribution),

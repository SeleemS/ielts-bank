 ...
 * authored under scripts/content/data/rdD-*.json ONLY. Owns the `rdD-` file
 * prefix so it can run concurrently with the shared import-batch.mjs without
 * touching any other agent's files. Identical to import-rich.mjs except for the
 * file-glob lane (rdD- instead of rich-).
 *
 ...
 *
 * USAGE
 *   node --import ./scripts/_wspreload.mjs scripts/content/import-rdD.mjs [--dry-run]
...
const log = (...a) => console.log('[import-rdD]', ...a);
...
async function main() {
  ...
  const files = readdirSync(DATA).filter((f) => f.startsWith('rdD-') && f.endsWith('.json'));
...
main().catch((e) => {
  console.error('[import-rdD] fatal:', e);
  process.exit(1);
});

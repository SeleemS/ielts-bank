 ...
 * authored under scripts/content/data/rdC-*.json ONLY. Owns the `rdC-` file
 ...
 *
 * USAGE
 *   node --import ./scripts/_wspreload.mjs scripts/content/import-rdC.mjs [--dry-run]
...
const log = (...a) => console.log('[import-rdC]', ...a);
...
async function main() {
  ...
  const files = readdirSync(DATA).filter((f) => f.startsWith('rdC-') && f.endsWith('.json'));
...
main().catch((e) => {
  console.error('[import-rdC] fatal:', e);
  process.exit(1);
});

 ...
 * import-wr2.mjs
 * --------------
 * Self-contained, idempotent importer for the `wr2-` writing batch. Reads ONLY
 * scripts/content/data/wr2-*.json and upserts each writing prompt into Supabase
 * by a stable slug. Safe to re-run: same slug -> same passage row, refreshed
 * writing_details. Cloned from import-wr.mjs with only the filename lane changed
 * (wr- -> wr2-) so it runs independently and never touches the wr- batch.
 *
 * USAGE
 *   node --import ./scripts/_wspreload.mjs scripts/content/import-wr2.mjs [--dry-run]
...
const log = (...a) => console.log('[import-wr2]', ...a);
...
async function main() {
  ...
  const files = readdirSync(DATA).filter((f) => f.startsWith('wr2-') && f.endsWith('.json'));
...
main().catch((e) => { console.error('[import-wr2] fatal:', e); process.exit(1); });

-- 20260724040000_questions_answered_and_fixed_rhythm.sql
-- dashboard_overview: totals/prev_totals gain questions_answered (count of
-- question_answer events — the KPI is renamed from "practice submits"), and
-- hour_heatmap is pinned to the trailing 7 days regardless of the selected
-- range. Everything else unchanged.
...
totals as (
  select
    ...
    (select count(*) from ev where event = 'question_answer') as questions_answered,
    ...
    (select coalesce(avg(secs), 0)::int
       from sessions where page_views >= 1) as avg_session_secs
),
prev_totals as (
  select
    count(*) as events,
    ...
    count(*) filter (where event = 'question_answer') as questions_answered,
...
hour_heatmap as (
  -- Always the trailing 7 days, independent of the selected range: the
  -- weekly-rhythm card is a fixed recent-cadence view.

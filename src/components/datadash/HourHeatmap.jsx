...
import { T, SEQ, seqColor, fmtNum } from './theme';
  ...
  return (
    <div ref={wrapRef} className="relative">
      <svg viewBox="0 0 520 168" className="w-full" role="img" aria-label="Activity by hour of week (UTC)">
        {DAYS.map((day, index) => (
          <text key={day} x="26" y={30 + index * 20} fontSize="10" textAnchor="end" fill={T.muted}>
            {day}
          </text>
        ))}
        {[0, 3, 6, 9, 12, 15, 18, 21].map((hour) => (
          <text key={hour} x={34 + hour * 20 + 9} y="12" fontSize="9" textAnchor="middle" fill={T.muted}>
          ...
          Array.from({ length: 24 }, (__, hour) => {
            ...
            return (
              <rect
                ...
                height="18"
                rx="3"
                fill={value ? seqColor(value, grid.max) : T.surfaceRaised}
      ...
      <div className="mt-1 flex items-center gap-1.5 text-[10px]" style={{ color: T.muted }}>
        <span>quiet</span>

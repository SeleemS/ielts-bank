...
// Three aligned single-series panels sharing one x-axis and one crosshair —
// visitors/events/submits live on very different scales, so small multiples
  ...
  const onMove = (evt) => {
    ...
    show(
      ...
      <div>
        ...
        {ROWS.map((row) => (
          <TipRow key={row.key} line swatch={T.blue} label={row.label} value={fmtNum(pt[row.key])} />
        ))}
    ...
    <div ref={wrapRef} className="relative" onMouseMove={onMove} onMouseLeave={onLeave}>
      <svg width="100%" height={height} role="img" aria-label="Traffic over time">
        {ROWS.map((row, rowIndex) => {
          ...
          const yAt = (value) => top + ROW_H - (ROW_H - 14) * ((value || 0) / max) - 2;
          ...
          return (
            <g key={row.key}>
              <text x={PAD.left} y={top + 10} fontSize="10" fontWeight="600" fill={T.muted}>
                {row.label.toUpperCase()}
              </text>
              <text x={width - PAD.right} y={top + 10} fontSize="10" textAnchor="end" fill={T.muted}>
                {fmtNum(max)}
              </text>
              <line x1={PAD.left} y1={top + ROW_H} x2={width - PAD.right} y2={top + ROW_H} stroke={T.axis} />
              <line x1={PAD.left} y1={top + 14} x2={width - PAD.right} y2={top + 14} stroke={T.grid} />
              {area && <path d={area} fill={T.blue} opacity="0.1" />}
              {line && <path d={line} fill="none" stroke={T.blue} strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />}
        ...
        {points.map((pt, i) =>
          i % tickEvery === 0 ? (
            <text key={pt.t} x={xAt(i)} y={height - 6} fontSize="10" textAnchor="middle" fill={T.muted}>
              {fmtTick(pt.t)}
            </text>
          ) : null
        )}

...
// Conversion funnel: distinct visitors reaching each stage in the range.
// One series over ordered stages → single hue; every stage direct-labeled.
        ...
        return (
          <div key={stage.key}>
            <div className="mb-0.5 flex items-baseline justify-between text-[12px]">
              <span style={{ color: T.ink2 }}>{stage.label}</span>
            ...
            <div className="h-[10px] overflow-hidden rounded-full" style={{ background: T.surfaceRaised }}>
              <div
                className="h-full rounded-full transition-[width] duration-500"
                style={{ width: `${Math.max(0.8, (100 * value) / top)}%`, background: T.blue }}
              />
            </div>
          </div>
        );

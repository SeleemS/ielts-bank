...
export default function PracticeFeedbackEntry({ skill, source = 'practice_index' }) {
  ...
  return (
    <section aria-label={`${speaking ? 'Speaking' : 'Writing'} feedback practice`} className="my-8 rounded-xl border border-accent/25 bg-accent/5 p-5 sm:p-6">
      ...
      <p className="mt-2 text-sm leading-6 text-muted-foreground">{speaking ? 'Choose a topic, record an answer and use your first Speaking sample score to choose a practice target.' : 'Bring an essay you have written and use your first Writing sample score to choose a practice target.'} One free sample per skill, per account. Already used yours? You can still practise for free; further AI scoring requires Pro.</p>
